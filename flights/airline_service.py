...
def airlines():
    ...
    if request.method == 'GET':
        airlines_list = perform_with_timeout(get_all_airlines)
        airlines = [{'id': airline.id, 'name': airline.name} for airline in airlines_list]
        return jsonify(airlines)
    elif request.method == 'POST':
        data = request.get_json()
        perform_with_timeout(create_airline, data['name'])
        return jsonify({'message': 'Airline created successfully'})
